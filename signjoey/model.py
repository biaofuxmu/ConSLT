...
class SignModel(nn.Module):
    ...
    def get_loss_for_batch(
        ...
        decoder_outputs2, gloss_probabilities2 = self.forward(
            sgn=batch.sgn,
            sgn_mask=batch.sgn_mask,
            sgn_lengths=batch.sgn_lengths,
            txt_input=batch.txt_input,
        ...
        if self.do_translation:
            ...
            word_outputs, hidden_output, _, _ = decoder_outputs
            ...
            txt_log_probs2 = F.log_softmax(word_outputs2, dim=-1) 
            ...
            hidden_output_flatten2 = hidden_output2.view(-1, hidden_output2.size(2))  
            ...
            nmt_loss2 = (
                translation_loss_function(txt_log_probs2, batch.txt)
                * translation_loss_weight
            ) # float
            translation_loss = (nmt_loss + nmt_loss2) / 2 + token_cl_loss * self.weight_cl
    ...
    def compute_token_infonce_loss(self, p, q, p_pos, q_neg, pad_mask=None):
        ...
        p_loss_unseq = p_loss.unsqueeze(1)
        q_loss_unseq = q_loss.unsqueeze(1)

        p_neg_loss = F.kl_div(F.log_softmax(p_pos, dim=-1), F.softmax(q_neg, dim=-1), reduction='none').mean(dim=-1)
        q_neg_loss = F.kl_div(F.log_softmax(q_neg, dim=-1), F.softmax(p_pos, dim=-1), reduction='none').mean(dim=-1)

        p_loss_ = torch.cat((p_loss_unseq, p_neg_loss), 1)
        q_loss_ = torch.cat((q_loss_unseq, q_neg_loss), 1)

        loss = (p_loss_ + q_loss_) / 2
...
def build_model(
    ...
    model: SignModel = SignModel(
        ...
        weight_cl=cfg.get("weight_cl", 0.5),
        neg_num=cfg.get("neg_num", 500),
    )
