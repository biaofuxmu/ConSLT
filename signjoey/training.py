...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict, args) -> None:
        """
        ...
        # self.model_dir = make_model_dir(
        #     args.model_dir, overwrite=train_config.get("overwrite", False)
        # )
        ...
        # self.last_best_lr = args.learning_rate
        print("last_best_lr: ", self.last_best_lr)
        self.learning_rate_min = train_config.get("learning_rate_min", 1.0e-8)
        self.clip_grad_fun = build_gradient_clipper(config=train_config)
        self.optimizer = build_optimizer(
            config=train_config, args=args, parameters=model.parameters()
        )
...
def train(args) -> None:
    ...
    trainer = TrainManager(model=model, config=cfg, args=args)

    trainer.logger.info("save model path :" + trainer.model_dir)
